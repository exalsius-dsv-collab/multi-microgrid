...
class MultiMicrogridLoadBalancer(vs.Controller):
    ...
    def __init__(self, microgrids: Dict[str, Dict], computing_nodes: Dict[str, List]):
        super().__init__()
    ...
    def step(self, time: int, p_delta: float, e_delta: float, state: dict) -> None:
        ...
        availability_scores = {} # Creates empty dictionary to store scores for each microgrid
        total_solar = 0 # Initializes counters for total solar and storage across all microgrids
        total_storage = 0
        
        for mg_name, mg_info in self.microgrids.items():
            # Get solar power from state
            solar_power = 0
            for actor_name, actor_data in state.items():
                if 'solar' in actor_name.lower() and mg_name.lower() in actor_name.lower():
                    solar_power = actor_data.get('p', 0)
                    break
            
            # Get battery state of charge
            battery_soc = 0
            for storage_name, storage_data in state.items():
                if 'battery' in storage_name.lower() and mg_name.lower() in storage_name.lower():
                    battery_soc = storage_data.get('soc', 0)
                    break
            
            # Calculate availability score (higher is better)
            # Solar power contributes 70%, battery SOC contributes 30%
            availability_score = (solar_power / 5000) * 0.7 + battery_soc * 0.3 # Divides by solar_power by5000W (maximum expected solar output) to normalize to 0-1 range
            availability_scores[mg_name] = max(0, availability_score)
            
            total_solar += solar_power
            total_storage += battery_soc
        
        # Normalize scores
        total_score = sum(availability_scores.values())
        if total_score > 0:
            normalized_scores = {k: v / total_score for k, v in availability_scores.items()} # Normalizes the scores by dividing by the total score
        else:
        ...
        for mg_name, nodes in self.computing_nodes.items():
            target_load_factor = normalized_scores.get(mg_name, 0.33) # Gets the normalized score for the microgrid, defaulting to 0.33 (since 3 microgrids) if not found
...
# Create monitors for each microgrid
monitors = {
    "berlin": vs.Monitor(outfile="results/berlin-microgrid.csv"),
    "san_francisco": vs.Monitor(outfile="results/san_francisco-microgrid.csv"),
    "sydney": vs.Monitor(outfile="results/sydney-microgrid.csv")
}

# Create a custom signal class that can be dynamically modified
...
# These will be Actor objects that can be dynamically controlled by the load balancer
computing_nodes = {
    "berlin": [
        vs.Actor(name="berlin_server_1", signal=AdjustableSignal(-400)),
        vs.Actor(name="berlin_server_2", signal=AdjustableSignal(-300)),
        vs.Actor(name="berlin_server_3", signal=AdjustableSignal(-200))
    ],
    "san_francisco": [
        vs.Actor(name="san_francisco_server_1", signal=AdjustableSignal(-500)),
        vs.Actor(name="san_francisco_server_2", signal=AdjustableSignal(-400)),
        vs.Actor(name="san_francisco_server_3", signal=AdjustableSignal(-300)),
        vs.Actor(name="san_francisco_server_4", signal=AdjustableSignal(-200))
    ],
    "sydney": [
        vs.Actor(name="sydney_server_1", signal=AdjustableSignal(-350)),
        vs.Actor(name="sydney_server_2", signal=AdjustableSignal(-250)),
        vs.Actor(name="sydney_server_3", signal=AdjustableSignal(-150))
    ]
}

# Create microgrid configurations
microgrid_configs = {}

# Berlin Microgrid
berlin_actors = [
    # Computing systems (individual nodes that can be controlled by load balancer)
    computing_nodes["berlin"][0],  # berlin_server_1
    computing_nodes["berlin"][1],  # berlin_server_2
    computing_nodes["berlin"][2],  # berlin_server_3
    
    # Solar generators using Berlin data
    vs.Actor(
        name="berlin_solar_1",
        signal=vs.Trace.load("solcast2022_global", column="Berlin", params={"scale": 3000}) # Scales the solar power to 3000W
    ),
    vs.Actor(
        name="berlin_solar_2", 
        signal=vs.Trace.load("solcast2022_global", column="Berlin", params={"scale": 2500})
    ),
    vs.Actor(
        name="berlin_solar_3",
        signal=vs.Trace.load("solcast2022_global", column="Berlin", params={"scale": 2000})
    )
]

# Combined storage for Berlin (3500Wh total capacity)
berlin_storage = vs.SimpleBattery(capacity=3500, initial_soc=0.65, min_soc=0.1)

# Create a single global load balancer controller
# Note: This "global" load balancer will be added to Berlin microgrid but will coordinate all microgrids using the microgrid_configs dictionary
global_load_balancer = MultiMicrogridLoadBalancer(
    microgrids=microgrid_configs,
    computing_nodes=computing_nodes
)

environment.add_microgrid(
    actors=list(berlin_actors),  # type: ignore
    storage=berlin_storage,
    controllers=[monitors["berlin"], global_load_balancer],
    step_size=300  # 5-minute steps
)

microgrid_configs["berlin"] = {
    "actors": berlin_actors,
    "storage": berlin_storage,
    "location": "Berlin"
}

# San Francisco Microgrid
sf_actors = [
    # Computing systems (individual nodes that can be controlled by load balancer)
    computing_nodes["san_francisco"][0],  # san_francisco_server_1
    computing_nodes["san_francisco"][1],  # san_francisco_server_2
    computing_nodes["san_francisco"][2],  # san_francisco_server_3
    computing_nodes["san_francisco"][3],  # san_francisco_server_4
    
    # Solar generators using San Francisco data
    vs.Actor(
        name="san_francisco_solar_1",
        signal=vs.Trace.load("solcast2022_global", column="San Francisco", params={"scale": 3500})
    ),
    vs.Actor(
        name="san_francisco_solar_2",
        signal=vs.Trace.load("solcast2022_global", column="San Francisco", params={"scale": 3000})
    ),
    vs.Actor(
        name="san_francisco_solar_3",
        signal=vs.Trace.load("solcast2022_global", column="San Francisco", params={"scale": 2500})
    )
]

# Combined storage for San Francisco (4500Wh total capacity)
sf_storage = vs.SimpleBattery(capacity=4500, initial_soc=0.75, min_soc=0.15)

environment.add_microgrid(
    actors=list(sf_actors),  # type: ignore
    storage=sf_storage,
    controllers=[monitors["san_francisco"]],
    step_size=300
)

microgrid_configs["san_francisco"] = {
    "actors": sf_actors,
    "storage": sf_storage,
...
sydney_actors = [
    # Computing systems (individual nodes that can be controlled by load balancer)
    computing_nodes["sydney"][0],  # sydney_server_1
    computing_nodes["sydney"][1],  # sydney_server_2
    computing_nodes["sydney"][2],  # sydney_server_3
    
    # Solar generators using Sydney data
    vs.Actor(
        name="sydney_solar_1",
        signal=vs.Trace.load("solcast2022_global", column="Sydney", params={"scale": 4000})
    ),
    vs.Actor(
        name="sydney_solar_2",
        signal=vs.Trace.load("solcast2022_global", column="Sydney", params={"scale": 3500})
    ),
    vs.Actor(
        name="sydney_solar_3",
        signal=vs.Trace.load("solcast2022_global", column="Sydney", params={"scale": 3000})
    )
]

# Combined storage for Sydney (5500Wh total capacity)
sydney_storage = vs.SimpleBattery(capacity=5500, initial_soc=0.7, min_soc=0.1)

environment.add_microgrid(
    actors=list(sydney_actors),  # type: ignore
    storage=sydney_storage,
    controllers=[monitors["sydney"]],
    step_size=300
)

microgrid_configs["sydney"] = {
    "actors": sydney_actors,
    "storage": sydney_storage,
    "location": "Sydney"
}
...
def calculate_summary_stats(df: pd.DataFrame, location: str) -> Dict:
    ...
    total_solar = df[solar_columns].sum(axis=1) if solar_columns else pd.Series(0, index=df.index)
    
    return {
        "location": location,
        "total_solar_energy": total_solar.sum() * 300 / 3600,  # Convert to kWh (300s steps)
        ...
        "renewable_percentage": (total_solar.sum() / (total_solar.sum() - df['p_delta'].sum())) * 100 if (total_solar.sum() - df['p_delta'].sum()) > 0 else 0
    }
...
def calculate_load_balancing_metrics(berlin_df: pd.DataFrame, sf_df: pd.DataFrame, sydney_df: pd.DataFrame):
    ...
    metrics = {
        "Berlin": {
            ...
            "grid_dependency": (berlin_grid[berlin_grid < 0].sum() * 300 / 3600),  # kWh drawn from grid
            "renewable_energy_exported": (berlin_grid[berlin_grid > 0].sum() * 300 / 3600),  # kWh exported
            "battery_utilization": berlin_df['storage.soc'].std(),  # Higher std = more dynamic usage
            "peak_solar_utilization": berlin_solar.max() / berlin_load.mean(),
            "load_variability": berlin_load.std() / berlin_load.mean()  # Coefficient of variation
        },
        "San Francisco": {
        ...
        "Sydney": {
            ...
            "renewable_energy_exported": (sydney_grid[sydney_grid > 0].sum() * 300 / 3600),
...
for location, metrics in load_balancing_metrics.items():
    ...
    print(f"  • Renewable Energy Exported: {metrics['renewable_energy_exported']:.1f} kWh")
...
total_renewable_export = sum(m['renewable_energy_exported'] for m in load_balancing_metrics.values())
avg_solar_ratio = np.mean([m['solar_load_ratio'] for m in load_balancing_metrics.values()])
...
print(f"Results saved to: results/berlin-microgrid.csv, results/san_francisco-microgrid.csv, results/sydney-microgrid.csv")
